...
def render():
    ...
    aAng = np.radians(alpha)
    bAng = np.radians(beta)
    gAng = np.radians(gamma)
    M = np.identity(4)
    Ra = Ra = np.array([[np.cos(aAng), -np.sin(aAng), 0],
                   [np.sin(aAng), np.cos(aAng), 0],
                   [0,0,1]]                       )
    Rb = np.array([[1,0,0],
                   [0, np.cos(bAng), -np.sin(bAng)],
                   [0, np.sin(bAng), np.cos(bAng)]])
    Rg = np.array([[np.cos(gAng), -np.sin(gAng), 0],
                   [np.sin(gAng), np.cos(gAng), 0],
                   [0,0,1]]                        )
